...
[McpServerToolType]
public static class ConvertToMarkdownTool
{
    [McpServerTool, Description(
        "Converts a document to clean, structured Markdown (.md). " +
        "Supports PDF, DOCX/DOC/RTF/ODT, XLSX/XLS/ODS/CSV/TSV, EPUB, MOBI, TXT, CHM and 20+ more formats. " +
        ...
        "Returns the saved file path followed by the Markdown content (long output is truncated inline — the saved .md holds the full text). " +
        ...
        "On failure, the response text starts with 'Conversion to Markdown failed for' followed by the underlying exception type, message, and inner chain.")]
        ...
        [Description("Password for protected documents")] string? password = null)
    {
        ...
        try
        {
            ...
            if (!result.IsSuccess)
                return $"Conversion to Markdown failed for '{resolved.FileName}': {result.ErrorMessage ?? "unknown error"}";
            ...
            var prefix = licenseManager.IsLicensed
                ? string.Empty
                : "[Evaluation mode] Output may be limited and include an evaluation notice.\n\n";
    ...
    private static string FormatException(Exception ex, string fileName)
    {
        ...
        sb.Append($"Conversion to Markdown failed for '{fileName}': ");
